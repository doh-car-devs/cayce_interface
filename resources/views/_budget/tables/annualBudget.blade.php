<table id="annualbudgettable" class="table table-bordered table-sm table-hover">
        ...
        @if ($item['parent_type_abbr'] == 'GAA')
            <tr>
                ...
                <td>
                    @foreach ($data['allocatedTotals'] as $i)
                        @if ($i['fund_source_id'] == $item['fund_source_id'])
                            ₱ {{number_format($item['NEP'] - $i['annualAllocatedNEP'], 2, '.', ',')}}
                        @endif
                    @endforeach
                </td>
    ...
    <tfoot>
        <tr>
            <th colspan="1" style="text-align:right">Filtered NEP Total / Total of currently visible entries:</th>
            <th colspan="4"style="text-align:left" id="total-id"></th>
        </tr>
        <tr>
            <th colspan="1" style="text-align:right">NEP Grand Total:</th>
