<table id="saroannualbudgettable" class="table table-bordered table-sm table-hover">
    <thead class="thead-dark">
        <tr>
            ...
            <th colspan="3" class="text-center">SARO Allocation<small>(SAA)</small></th>
            <th rowspan="2" class="text-right" style="width: 10%">Take Action</th>
        </tr>
        <tr>
            <th>SARO Allocation Status<small>(SARO)</small></th>
            <th>Allocated</th>
            <th>Unallocated</th>
        </tr>
    </thead>
        ...
        @foreach ($data['annual'] as $item)
            ...
            <tr>
                ...
                <td>
                    @foreach ($data['allocatedTotals'] as $i)
                        @if ($i['fund_source_id'] == $item['fund_source_id'])
                            ₱ {{number_format($item['amount'] - $i['annualAllocatedAmount'], 2, '.', ',')}} 
                        @endif
                    @endforeach
                </td>
